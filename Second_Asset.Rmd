...
# Number of Negative/Positive interviews Experience in the top 10 job openings in Amazon
```{r}
y <- table(result$title, result$Experience_outcome)
y <- tail(y[order(rowSums(y)),],n=10)
y <- t(y)
par(mar=c(12,4,1,0.1))
barplot(y, col = 2:3, legend = c("Negative", "Positive"), args.legend = list(x ='top'), ylab = "Counts", cex.axis=1.0, cex.name=0.9, las=2, lwd=2, beside=TRUE)
```

# Number of Accepted/Rejected interviews in the top 10 Amazon's offices
```{r}
...
# Number of Interview Harship Level in the top 10 top job openings in Amazon
```{r}
y <- table(result$title, result$Interview_outcome)
y <- tail(y[order(rowSums(y)),],n=10)
y <- t(y)
par(mar=c(12,4,1,0.1))
barplot(y, col = c("blue","red","green"), legend = c("Average", "Difficult", "Easy"), args.legend = list(x ='top'), ylab = "Counts", cex.axis=1.0, cex.name=0.9, las=2, lwd=2, beside=TRUE)
```